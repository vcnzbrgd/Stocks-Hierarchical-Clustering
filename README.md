...
- Build an Ultrametric Distance $d'(i,j)$ starting from $d(i,j)$. EX: the Ultrametric distance between $A$ and $G$ is defined as the maximum value among all the pairwise distances evaluated in the path that connects A and G i.e. $d'(A,G) = 8$ as $max(3,2,8,7,4,1,3)=8$ 
